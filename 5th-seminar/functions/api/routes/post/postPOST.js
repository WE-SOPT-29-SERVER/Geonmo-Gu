...
module.exports = async (req, res) => {
  ...
  if (!userId || !title || !content) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
  ...
  try {
    ...
    const newPost = await postDB.createPost(client, title, content, userId);
    // 성공적으로 users를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.ADD_ONE_POST_SUCCESS, newPost));

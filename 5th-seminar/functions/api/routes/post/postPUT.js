...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const newPost = await postDB.updatePost(client, postId, title, content);
    // 성공적으로 users를 가져왔다면, response를 보내줍니다.

    if(!newPost) return res.status(statusCode.NOT_FOUND).send(statusCode.NOT_FOUND, responseMessage.NO_POST);
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.UPDATE_ONE_POST_SUCCESS, newPost));
